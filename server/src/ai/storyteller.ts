...
export function validateDecision(room: Room, stepId: string, decision: unknown): StorytellerDecision | null {
  ...
  if (stepId === 'imp') {
    ...
    if (demon?.seatIndex === targetSeatIndex) return null;
 ...
 */
async function callOpenAI(req: StorytellerRequest, stepId: string): Promise<unknown> {
  ...
  const userPrompt = isTwoPlayersOneChar
    ? `剧本：${req.scriptNameZh}。当前为第${req.dayNumber}天夜晚，步骤：${getStepNameZh(stepId)}。需要选择两名存活玩家（座位号）和其中一个角色身份。存活玩家座位号：${req.aliveSeatIndices.join(',')}。可选善良角色ID（从剧本中选一个）：${req.scriptNameZh}镇民/外来者。回复格式：{"players":[座位1,座位2],"characterId":"角色id"}`
    : `剧本：${req.scriptNameZh}。恶魔选择一名玩家杀害。存活玩家座位号：${req.aliveSeatIndices.join(',')}。回复格式：{"targetSeatIndex":座位号}`;
