...
import type { StorytellerDebugEvent } from './storyteller.js';

export type GodAgentSendNightInfo = (roomId: string, room: Room, seatIndex: number, message: string) => void;

export interface GodAgentTickOptions {
  sendNightInfo: GodAgentSendNightInfo;
...
export class GodAgent {
  ...
  private sentInfoOnce(room: Room, stepId: string, seatIndex: number): boolean {
  ...
  async tickNight(room: Room, options: GodAgentTickOptions): Promise<void> {
    const { sendNightInfo } = options;
    const maxStepsPerRun = Math.max(1, Math.min(20, Number(options.maxStepsPerRun ?? 1)));
    let advancedSteps = 0;

    for (;;) {
      if (advancedSteps >= maxStepsPerRun) break;
      if (room.pendingNightAction) break;
      if (room.awaitingNightInfoConfirm) break;
      ...
      if (stepId === 'ravenkeeper') {
        for (const vSeat of room.lastNightDeaths) {
          ...
          if (msg) sendNightInfo(this.roomId, room, vSeat, msg);
        }
        room.nightStepIndex++;
        advancedSteps++;
        break;
...
const GOD_AGENT_BY_ROOM = new Map<string, GodAgent>();

export function getOrCreateGodAgent(roomId: string): GodAgent {
