...
function buildSystemPromptModules(params) {
    ...
    const stageConstraint = params.stage === 'night_action'
        ? [
...
function heuristicSummary(previousSummary, recentLines, maxChars) {
...
function seatBeliefInit() {
    return { suspicion: 0.5, trust: 0.5, tags: [] };
}
function normalizeLine(s) {
    return String(s ?? '').replace(/\s+/g, ' ').trim();
}
function makeSeenKey(parts) {
    return parts.map((x) => String(x ?? '')).join('|');
}
function extractSeatRefs(text) {
    const s = String(text ?? '');
    const out = new Set();
    // 形如：#3、#12
...
function pushTagUnique(tags, tag) {
    if (!tag)
        return;
...
export class PlayerSeatAgent {
     ...
     */
    ingestFromContext(ctx) {
        const now = Date.now();
        // 1) 吸收可见聊天（只用 ctx.chatLog；不吃 allChatLog，避免越权全知）
        const chat = Array.isArray(ctx.chatLog) ? ctx.chatLog : [];
        for (const e of chat) {
            const scope = String(e.scope ?? '');
            const fromSeat = Number(e.fromSeat);
            const toSeat = e.toSeat;
            const at = Number(e.at ?? now);
            const text = normalizeLine(String(e.text ?? '').slice(0, 500));
            if (!Number.isInteger(fromSeat) || !text)
                continue;
            ...
            if (refs.length > 0) {
                ...
                for (const seatRef of refs) {
                    if (!Number.isInteger(seatRef) || seatRef < 0)
                        continue;
        ...
        if (ctx.voteSnapshot) {
            ...
            for (const v of votes) {
                const s = Number(v.seatIndex);
                const inFavor = !!v.inFavor;
        ...
        if (this.state.timeline.length > 420)
            this.state.timeline = this.state.timeline.slice(-420);
        ...
        for (const n of nominations.slice(-12)) {
            const nominator = n.nominator;
            const nominated = n.nominated;
            if (!Number.isInteger(nominator) || !Number.isInteger(nominated))
                continue;
    ...
    async callJsonMode(params) {
        ...
        const debugStage = params.debugStage
        ...
        if (!content)
            return { content: null, err: 'empty_response' };
        params.onDebug?.({
            stage: debugStage,
            kind: 'response',
            seatIndex: this.seatIndex,
     ...
     */
    async decideDayPlan(ctx, temperature, onDebug) {
        if (!this.apiKey || !this.useEnabled)
            return { systemPrompt: '', userPrompt: '', raw: null, err: 'disabled' };
     ...
     */
    async decideNightTargets(ctx, temperature, onDebug) {
        if (!this.apiKey || !this.useEnabled)
            return { systemPrompt: '', userPrompt: '', raw: null, err: 'disabled' };
        if (!ctx.nightPrompt)
            return { systemPrompt: '', userPrompt: '', raw: null, err: 'no_night_prompt' };
        this.ingestFromContext(ctx);
     ...
     */
    async decideSingleAction(ctx, temperature) {
        if (!this.apiKey || !this.useEnabled)
            return { systemPrompt: '', userPrompt: '', raw: null, err: 'disabled' };
        this.ingestFromContext(ctx);
     ...
     */
    async decideConstrainedAction(params, onDebug) {
        if (!this.apiKey || !this.useEnabled)
            return { systemPrompt: '', userPrompt: '', raw: null, err: 'disabled' };
...
export function getOrCreateSeatAgent(room, seatIndex, create) {
    const key = `ai_player_agent_v1_${seatIndex}`;
    const v = room.storytellerDecisions.get(key);
    if (v instanceof PlayerSeatAgent)
        return v;
    const a = create();
