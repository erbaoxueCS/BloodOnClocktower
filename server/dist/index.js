...
function sendNightInfo(roomId, room, seatIndex, message) {
    ...
    if (!(v instanceof Set))
        room.storytellerDecisions.set(dk, set);
...
function maybeLogDawn(room, phaseBefore) {
    ...
    if (room.storytellerDecisions.get(mk) === true)
        return;
...
const lastDirectorDebugAt = new Map();
...
function maybeLogDirectorDebug(roomId, room) {
    if (!AI_DIRECTOR_LOG || room.status !== 'playing')
        return;
    const now = Date.now();
...
function orderedAliveSeats(room, startSeat) {
    const alive = room.players.filter((p) => p.isAlive).map((p) => p.seatIndex).sort((a, b) => a - b);
    if (alive.length === 0)
        return [];
    if (startSeat == null || !alive.includes(startSeat))
        return alive;
...
function getDayFlowCursor(room, stage) {
    const key = `day_flow_cursor_${room.dayNumber}_${stage}`;
    return Number(room.storytellerDecisions.get(key) ?? 0);
}
function setDayFlowCursor(room, stage, cursor) {
    const key = `day_flow_cursor_${room.dayNumber}_${stage}`;
    room.storytellerDecisions.set(key, cursor);
}
function dayPlanTraceKey(dayNumber, seatIndex) {
    return `ai_day_plan_trace_id_${dayNumber}_${seatIndex}`;
}
function appendBehavior(roomId, room, seatIndex, traceId, part) {
    if (!traceId)
        return;
    const prev = room.storytellerDecisions.get(`${traceId}_behavior`) ?? '';
...
function moveToNextDayFlowStage(room) {
    ...
    if (next === 'nomination_vote') {
        ...
        const stageZh = next === 'private_dialogue'
            ? '玩家私聊'
            : next === 'public_speech'
                ? '公开发言'
                : '上帝问答';
...
function maybeAdvanceStructuredDay(roomId, room) {
    if (room.phase !== 'day' || room.status !== 'playing')
        return;
    if (room.dayFlowStage == null)
        return;
    if (room.dayFlowStage === 'nomination_vote')
        return;
    if (room.currentNomination)
        return;
...
function fillMissingVotesAsAgainst(room) {
...
function skipRemainingAlivePlayers(room) {
    const alive = room.players.filter((p) => p.isAlive).map((p) => p.seatIndex);
    let skipped = 0;
    for (const seat of alive) {
        if (room.nominationsToday.has(seat) || room.skippedNominationsToday.has(seat))
            continue;
...
function enforceProgressFallback(roomId, room) {
    if (room.status !== 'playing')
        return;
    const now = Date.now();
...
const nightLoopInFlightByRoom = new Map();
...
async function runNightLoopExclusive(roomId, room) {
...
function getAiSharedNightInfoCursor(room) {
...
function getAiTrustScores(room, viewerSeatIndex) {
...
function getRoleAlignmentByNameZh(room, roleZh) {
    const c = room.script.characters.find((x) => x.nameZh === roleZh);
    if (!c)
        return null;
    return (c.alignment ?? 'good');
}
function parseNightInfoToTrustDeltas(room, infoText) {
    ...
    if (infoText.includes('你是恶魔') || infoText.includes('你是爪牙')) {
        ...
        const best = new Map();
        for (const d of out)
            best.set(d.seatIndex, (best.get(d.seatIndex) ?? 0) + d.delta);
        return Array.from(best.entries()).map(([seatIndex, delta]) => ({ seatIndex, delta }));
    }
    return out;
}
function updateAiTrustFromSharedNightInfo(room, infoText) {
    ...
    for (const viewerSeatIndex of aiSeats) {
        ...
        for (const d of deltas) {
            if (!Number.isInteger(d.seatIndex))
                continue;
...
function aiPickNominationTargetByTrust(room, viewerSeatIndex) {
    ...
    if (worstScore > inFallbackThreshold)
        return null;
    return worstSeat;
}
function aiVoteInFavorByTrust(room, voterSeatIndex, nominatedSeatIndex) {
    const inFallbackThreshold = Number(process.env.AI_FALLBACK_TRUST_SUSPECT_THRESHOLD ?? -1);
...
function pickTopSuspiciousAlive(room, viewerSeatIndex, limit = 2) {
    const scores = getAiTrustScores(room, viewerSeatIndex);
...
function summarizeMyNightInfo(room, seatIndex) {
...
function buildForcedActiveDayPlan(room, seatIndex) {
    ...
    if (target != null) {
        for (const p of room.players) {
            if (!p.isAlive || p.seatIndex === seatIndex || p.seatIndex === target)
                continue;
            dm.push({
                toSeat: p.seatIndex,
                text: rolePush
...
function isWeakPublicSpeech(text) {
    const t = String(text ?? '').trim();
    if (!t)
        return true;
    if (t.length >= 40 && (t.includes('我的夜间信息') || t.includes('我昨晚') || t.includes('#')))
        return false;
    const weakPatterns = [
        '我先观望',
        '简单交换信息',
...
function buildRecentVoteEvents(room) {
...
function shouldVoteInFavorByPriority(room, seatIndex, nominatedSeat, explicitPriority) {
    ...
    if (priority.includes(nominatedSeat))
        return true;
