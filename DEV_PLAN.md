...
每次提交（commit）前必须至少完成以下事项，避免“功能能跑但协作者接不住”：

- **整理改动摘要**：把本次改动的高层目标、关键行为变化、边界变化写入本文件的「最近改动归档」。
- **同步整体设计文档**：若改动影响了 AI 链路/流程编排/可观测性/权限边界，需同步更新 `docs/` 下对应设计文档（或新增文档）并在本文件中引用。
- **检查边界回归**：确认以下语义没有被破坏：
  - `aiStorytellerEnabled` 只控制“说书人/流程导演是否自动推进”，不应影响 AI 玩家托管是否可行动与互动；
  - 提名/投票/夜间行动必须走统一裁决入口并可追踪；
  - 私聊可见性严格隔离：只对双方可见，不能被“全部视图/公屏”混入展示。
- **避免提交构建产物**：不要把 `client/dist/*`、`server/dist/*`、`.DS_Store`、`.cursor/` 等本地文件提交到仓库。
...
## 13. 最近改动归档（2026-04-28）

### 13.1 白天/夜晚流程与互动链路修复（根因）

- **白天结构化阶段初始化**：进入白天时初始化 `dayFlowStage=god_dialogue`、`daySubPhase=discussion`，并设置随机起始座位，保证“上帝问答→私聊→公聊→提名投票”阶段可跑且可观测。
- **结构化阶段推进与 AI 说书人解耦**：白天结构推进由“是否存在 AI 托管玩家”驱动，避免 AI 玩家互动链路被 `aiStorytellerEnabled` 门禁误伤；同时保留“强兜底快进”仍由 `aiStorytellerEnabled` 控制。
- **阶段完成更严格**：`god_dialogue/private_dialogue/public_speech` 必须出现“实际行为”或写入“超时跳过”记录后才能推进，避免“阶段走完但无任何聊天记录”。
- **私聊必有回放**：`private_dialogue` 若模型计划未产出私聊，系统至少补发 1 条“有理由的询问/协作”私聊，确保 AI-AI 互动可回溯（不再出现私聊全空）。

### 13.2 聊天回溯与可读性（前端）

- **对话区统一回溯**：对局结束后保留对话回溯面板，支持查看公聊/上帝/私聊。
- **私聊严格归位**：`全部` 视图只展示 `public + god`，私聊只在 `私聊` 页按“你 ↔ 目标玩家”查看，避免在全量里混入私聊箭头扰乱阅读。

### 13.3 AI 玩家话术与行为方式模块（替代“积极程度”）

- **行为方式合并模块**：将“语气画像 + 积极程度温度”合并为 `aiPlayerBehaviorStyle`（每座位每局固定/可手动切换），并注入后续 AI 调用，影响：
  - `voiceProfile`（表达画像/句式偏好）
  - `promptStyle`（balanced/assertive/deceptive/chaotic 的策略倾向）
  - 内部 `temperature` 映射（仅内部使用，不再对外暴露）
